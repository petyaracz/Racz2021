...
title: Identifying covariate traits of trance and possession phenomena in the Ethnographic
  Atlas using multiple imputation and nested sparse regression
author: "Péter Rácz"
...
The existing literature identifies a number of correlates of trance states in the Ethnographic Atlas. We select twenty-one of these as starting variables. Categorical variables will be fit with a default intercept, ordinal variables, where we can posit a hierarchy between factor levels, will be fit as numeric scales. This runs the risk of missing non-linear effects, but contributes to model health.
...
This is good enough for the figure. We will take data imputation more seriously in the models.

We visualise Spearman correlations between the relevant variables in our imputed dataset. We can see that some of these are strongly correlated. For example, primary class differentiations in a society (EA066) increase with the increase of structure in the larger community (EA033). Different primary subsistence economies (EA042) go together with different levels of local and larger social hierarchical complexity. This makes sense.
...
For the purposes of this broad-brush analysis, we recognise a number of implicative structures. Societies can be grouped according to whether:
...
dw1 = dw1 %>% 
  mutate(
    ...
    `trance and possession` = case_when(
      ...
      trance == 4 ~ "P->T",
      trance == 5 ~ "P=>T"
    ) %>% factor(levels = c("¬(P||T)","P||T","P&T","P->T","P=>T"))
  )

ggplot() + 
  geom_polygon(data = mapWorld, aes(x=long, y = lat, group = group, alpha = .3)) + 
  geom_text(data = dw1 %>% filter(!is.na(`trance and possession`)), aes(x = lon2, y = lat, colour = `trance and possession`, label = trance)) +
...
Possession and/or Trance states are posited (2-3) primarily in the Americas in the dataset, whereas strong links are present for the two (4 and especially 5) primarily in Sub-Saharan Africa and Oceania.

Data are missing for `r dw1 %>% filter(is.na(trance)) %>% nrow()` societies in the EA data.

## Modelling: coding the outcome

Without making a deep ontological commitment to this hierarchy, we can use it to model the distribution of trance and possession beliefs in the dataset by using an ordered categorical model. This is an extension of the logistic generalised linear regression model which predicts the relative likelihood of one outcome (say, presence of trance states reported in society) over another (say, absence of trance states).
...
samples = samples %>% 
  mutate(
    pots = map(model, ~ as_tibble(summary(., prob = .67)$fixed, rownames = 'term'))
  )
samples_67 = samples %>% 
  select(id, pots) %>% 
  unnest(cols = pots)  %>% 
...
write_tsv(samples_67, 'data/imputed_67.tsv')
...
As we see, most predictors have relatively little to contribute in this sparse model.

We can look at the individual posterior distributions of the ten models as well.

```{r robustness_iterations3a, fig.height = 12}
pots = read_tsv('data/imputed_67.tsv')

pots = pots %>% 
  mutate(
    predictor = reorder(term, Estimate)
    )

ggplot(pots, aes(x = predictor, y = Estimate, ymin = `l-67% CI`, ymax = `u-67% CI`, group = id)) + 
  geom_point(position = position_dodge(width = .75)) +
  geom_linerange(position = position_dodge(width = .75)) +
  geom_hline(yintercept = 0, lty = 2) +
  coord_flip()

```

This is the same plot of posteriors ordered by absolute median, except that each row plots ten distributions instead of one. The bars show 67% credible intervals. This is a dense figure, but what the reader can garner is that most variation is visible for those predictors at the top. These are the ones that generally have non-zero effects.

The ten models are identical except for the datasets on which they were fit. The missing values of the EA data were imputed by @thompson2018quantifying for each imputed dataset. Another way in which we can assess the overall effects across these data is by pooling the draws from the posterior distributions across models. The result can be seen below.
...
Altogether, across the ten models fit on the ten imputed datasets, most of our predictor variables explain little variation in the outcome. 
...
variables_dict_keep = variables_dict %>% 
  ...
  distinct(var_title, var_id, var_title2)

variables_dict_keep = pots %>% 
  select(term, `l-67% CI`, `u-67% CI`) %>% 
  rename(var_title2 = term) %>% 
  filter(var_title2 %in% variables_dict_keep$var_title2) %>% 
  group_by(var_title2) %>% summarise(
    lower_67 = mean(`l-67% CI`),
    upper_67 = mean(`u-67% CI`)
  ) %>% 
  left_join(variables_dict_keep)
  
variables_dict_keep %>% 
  select(var_id, var_title, lower_67, upper_67) %>% 
...
We are now done with variable selection and want to see how well a model with the surviving predictor variables predicts the real data.

We refit the model on the complete observations of the EA (`r nrow(dw4)`/`r nrow (dw2)` societies). We use weakly informative student-t priors for both betas and intercepts. 
...
hf = hypothesis(fit4, 'Settlement_patterns>0', alpha = 0.33)$hypothesis

bind_rows(ha,hb,hc,hd,he,hf) %>% 
  select(Hypothesis, Estimate, Est.Error, CI.Lower, CI.Upper) %>% 
  mutate(Hypothesis = str_replace_all(Hypothesis, '[_]', ' ') %>% str_replace_all('[)(]', '')) %>% 
  bind_cols(alpha = c(0.03, 0.03, 0.03, 0.33, 0.33, 0.33)) %>% 
...
We perform two further robustness checks by filtering the dataset.
...
Some effects shrink in the refit models, but the differences are not very large.

## Visualising predictions

We take each predictor and draw predictions from Fit 4 (our favourite fit) for societies at each of its levels. Then, we calculate the mean and the standard deviation and plot these against each other, levels of the predictor on the x axis, predictions for the five levels of the outcome on the y axis. The resulting plot needs some work in Illustrator.

```{r predictions, fig.width = 9, fig.height = 9}

predPlot = function(variable_name){
  
  dict = variables_dict %>% 
  mutate(
    var_name = case_when(
      recoded_value == 'patrilineal' ~ 'patrilineal',
      recoded_value == 'other' ~ 'other',
      T ~ var_name
    )
  ) %>% 
  filter(
    var_title2 == variable_name,
    !is.na(code)
    )
  ...
  title = paste0(unique(dict$var_id), ':\n', unique(dict$var_title))
  
  pred_sum %>% 
  filter(var_title2 == variable_name) %>% 
  ggplot() +
  geom_line(aes(x = recoded_value2, y = mean)) +
  geom_line(aes(x = recoded_value2, y = mean + sd), colour = 'grey') +
  geom_line(aes(x = recoded_value2, y = mean - sd), colour = 'grey') +
  theme_bw() +
...
pred = dw4 %>% 
  mutate(
    Descent_major_type = case_when(
      Descent_major_type == 'patrilineal' ~ 1,
      Descent_major_type == 'other' ~ 2
    )
  ) %>% 
  select(
    soc_id,
  ...
  pivot_longer(- soc_id, names_to = 'var_title2', values_to = 'recoded_value2') %>% 
  left_join(pred, by = "soc_id") %>% 
  pivot_longer(- c(soc_id, var_title2, recoded_value2), names_to = 'level', values_to = 'p')

pred_sum = pred %>% 
  group_by(var_title2,recoded_value2,level) %>% 
  summarise(
    mean = mean(p),
    sd = sd(p)
            ) %>%
  mutate(
    level = factor(level, levels = c('no P, no T', 'P xor T', 'P and T', 'P -> T', 'P <-> T'))
  )

# Slavery_type                                        0.24      0.09     0.06     0.41 1.00
# Religion_high_gods                                 -0.29      0.09    -0.46    -0.12 1.00
# Jurisdictional_hierarchy_beyond_local_community     0.23      0.11     0.00     0.45 1.00
# Descent_major_typepatrilineal                       0.23      0.20    -0.16     0.63 1.00
# Settlement_patterns                                 0.05      0.05    -0.05     0.14 1.00
# Class_differentiation_primary                       0.06      0.09    -0.13     0.25 1.00
p1 = predPlot("Slavery_type") + min_theme
p2 = predPlot("Religion_high_gods") + min_theme
p3 = predPlot("Jurisdictional_hierarchy_beyond_local_community") + min_theme
p4 = predPlot("Descent_major_type") + min_theme
p5 = predPlot("Class_differentiation_primary") + min_theme
p6 = predPlot("Settlement_patterns") + right_theme + scale_y_continuous(limits = c(0,0.6), position = 'right', name = 'mean predicted probability')
